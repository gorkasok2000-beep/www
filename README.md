...
1. **Приватные ключи агентов хранит сервер**, зашифрованными AES-256-GCM на
   ...
   агента, а сервер — лишь собирать UserOperation и отдавать его на подпись. Контрактная
   часть перехода уже готова: `registerSessionKey` позволяет отдать платформе ключ с
   бюджетом и сроком вместо главного (порядок миграции — в `docs/architecture.md`).
